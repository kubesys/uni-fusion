package com.qnkj.clouds.modules.VmAffinityGroups.configs;
...
public class ConfigDataConfig extends BaseDataConfig {
	public ConfigDataConfig() {
		/*菜单配置*/
		moduleMenu = new ModuleMenu().builtin(true).program("kubestack").parent("CloudResources").label("亲和组").icon("iconfont icon-kucun").url("/VmAffinityGroups/listview").order(4);

		/*模块配置*/
		tabs = new Tab().modulename("VmAffinityGroups").modulelabel("亲和组").tabname("vm_affinitygroups").iscreate(true).isdelete(true).isenabledisable(true).customview(false).ishidden(true);

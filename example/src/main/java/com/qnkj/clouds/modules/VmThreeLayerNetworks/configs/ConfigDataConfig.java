package com.qnkj.clouds.modules.VmThreeLayerNetworks.configs;
...
public class ConfigDataConfig extends BaseDataConfig {
	public ConfigDataConfig() {
		/*菜单配置*/
		moduleMenu = new ModuleMenu().builtin(true).program("kubestack").parent("NetworkResources").label("三层网络").icon("iconfont icon-kucun").url("/VmThreeLayerNetworks/listview").order(4);

		/*模块配置*/
		tabs = new Tab().modulename("VmThreeLayerNetworks").modulelabel("三层网络").tabname("vm_threelayernetworks").iscreate(true).isdelete(true).isenabledisable(true).customview(false).ishidden(true);
		...
		/*模块API接口配置信息*/
		/*api.modulename("VmThreeLayerNetworks");*/

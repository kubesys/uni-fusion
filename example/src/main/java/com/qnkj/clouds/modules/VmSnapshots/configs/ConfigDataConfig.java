package com.qnkj.clouds.modules.VmSnapshots.configs;
...
public class ConfigDataConfig extends BaseDataConfig {
	public ConfigDataConfig() {
		/*菜单配置*/
		moduleMenu = new ModuleMenu().builtin(true).program("kubestack").parent("CloudResources").label("快照").icon("iconfont icon-kucun").url("/VmSnapshots/listview").order(9);

		/*模块配置*/
		tabs = new Tab().modulename("VmSnapshots").modulelabel("快照").tabname("vm_snapshots").iscreate(true).isdelete(true).isenabledisable(true).customview(false).ishidden(true);

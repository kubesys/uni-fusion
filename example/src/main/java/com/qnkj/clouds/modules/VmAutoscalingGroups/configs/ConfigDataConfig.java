package com.qnkj.clouds.modules.VmAutoscalingGroups.configs;
...
public class ConfigDataConfig extends BaseDataConfig {
	public ConfigDataConfig() {
		/*菜单配置*/
		moduleMenu = new ModuleMenu().builtin(true).program("kubestack").parent("CloudResources").label("弹性伸缩组").icon("iconfont icon-kucun").url("/VmAutoscalingGroups/listview").order(8);

		/*模块配置*/
		tabs = new Tab().modulename("VmAutoscalingGroups").modulelabel("弹性伸缩组").tabname("vm_autoscalinggroups").iscreate(true).isdelete(true).isenabledisable(true).customview(false).ishidden(true);

package com.qnkj.clouds.modules.VmPortForwardings.configs;
...
public class ConfigDataConfig extends BaseDataConfig {
	public ConfigDataConfig() {
		/*菜单配置*/
		moduleMenu = new ModuleMenu().builtin(true).program("kubestack").parent("NetworkServices").label("端口转发").icon("iconfont icon-kucun").url("/VmPortForwardings/listview").order(4);

		/*模块配置*/
		tabs = new Tab().modulename("VmPortForwardings").modulelabel("端口转发").tabname("vm_portforwardings").iscreate(true).isdelete(true).isenabledisable(true).customview(false).ishidden(true);
		...
		/*模块API接口配置信息*/
		/*api.modulename("VmPortForwardings");*/

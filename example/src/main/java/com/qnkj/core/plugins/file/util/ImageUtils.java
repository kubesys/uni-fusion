...
public class ImageUtils {
	...
	public static byte[] processImage(String fileId,byte[] img,int rotate,int quality,int resizeWidth,int resizeHeight,int cropWidth,int cropHeight,String fmt) throws IOException{
		if(img!=null){
			...
			try{
				if(!requireProcess(image, fileId, rotate, quality, resizeWidth, resizeHeight, cropWidth, cropHeight, fmt)){
					return img;
				}

				resizeWidth = fix(image.getWidth(),resizeWidth);
				if (resizeHeight == 0) {
					resizeHeight = fix(image.getHeight() * resizeWidth / image.getWidth(),resizeHeight);
				} else {

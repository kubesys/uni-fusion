package com.qnkj.clouds.modules.VmElasticIPs.configs;
...
public class ConfigDataConfig extends BaseDataConfig {
	public ConfigDataConfig() {
		/*菜单配置*/
		moduleMenu = new ModuleMenu().builtin(true).program("kubestack").parent("NetworkServices").label("弹性IP").icon("iconfont icon-kucun").url("/VmElasticIPs/listview").order(3);

		/*模块配置*/
		tabs = new Tab().modulename("VmElasticIPs").modulelabel("弹性IP").tabname("vm_elasticips").iscreate(true).isdelete(true).isenabledisable(true).customview(false).ishidden(true);

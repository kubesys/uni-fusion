package com.qnkj.clouds.modules.VmVirtualIPs.configs;
...
public class ConfigDataConfig extends BaseDataConfig {
	public ConfigDataConfig() {
		/*菜单配置*/
		moduleMenu = new ModuleMenu().builtin(true).program("kubestack").parent("NetworkServices").label("虚拟IP").icon("iconfont icon-kucun").url("/VmVirtualIPs/listview").order(2);

		/*模块配置*/
		tabs = new Tab().modulename("VmVirtualIPs").modulelabel("虚拟IP").tabname("vm_virtualips").iscreate(true).isdelete(true).isenabledisable(true).customview(false).ishidden(true);

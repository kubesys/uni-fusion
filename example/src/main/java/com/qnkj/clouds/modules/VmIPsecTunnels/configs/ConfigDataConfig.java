package com.qnkj.clouds.modules.VmIPsecTunnels.configs;
...
public class ConfigDataConfig extends BaseDataConfig {
	public ConfigDataConfig() {
		/*菜单配置*/
		moduleMenu = new ModuleMenu().builtin(true).program("kubestack").parent("NetworkServices").label("IPsec隧道").icon("iconfont icon-kucun").url("/VmIPsecTunnels/listview").order(5);

		/*模块配置*/
		tabs = new Tab().modulename("VmIPsecTunnels").modulelabel("IPsec隧道").tabname("vm_ipsectunnels").iscreate(true).isdelete(true).isenabledisable(true).customview(false).ishidden(true);

...
@Slf4j
@Validated
@Controller("base-VmInstances")
@RequiredArgsConstructor
@Api(tags = "低代码演示：云主机")
@Scope("prototype")
@RequestMapping("VmInstances")
public class BaseModuleController {
    ...
    public Object ajaxIndex(HttpServletRequest request) {
        Map<String,Object> list = vminstancesService.getListViewEntity(request,viewEntitys, VmInstances.class);
    ...
    public Object editView(HttpServletRequest request,HttpServletResponse response, Model model) {
        ...
        VmInstances result = (VmInstances) vminstancesService.getObjectByRecord(httpRequest.get("record"),viewEntitys,VmInstances.class);
        if(result != null) {
    ...
    public Object editViewSave(HttpServletRequest request, Model model) {
        try{
            ...
            vminstancesService.save(httpRequest,viewEntitys,VmInstances.class);
    ...
    public Object delete(HttpServletRequest request, Model model) {
        try {
            Map<String, Object> httpRequest = Utils.getRequestQuery(request);
            vminstancesService.delete(httpRequest,viewEntitys);

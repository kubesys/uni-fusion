layui.define("form", function(exports) {
	...
	multiSelect.prototype.init = function() {
		...
		o('select[multiple]').each(function(idx, item) {
			...
			t.next().addClass('multi').find('.layui-select-title').click(function() {
				selected_titles[idx] && o(this).find('input').val(selected_titles[idx].join(','));
				...
				ts.selected(idx, t, dt);
				var filter = t.attr('lay-filter'); //获取过滤器
				layui.event.call(this, 'form', 'select('+ filter +')', {
					elem: t
					,value: selected_vals[idx].join(',')
					,othis: dt
				});
				return false;
			});
	...
	multiSelect.prototype.selected = function(idx, t, dt) {
		...
		dt.parents('dl').find('[type=checkbox]:checked').each(function() {
			...
			selected_titles[idx].push(o(this).attr('title'));
		})
		// 显示已选信息
		dt.parents('dl').prev().find('input').val(selected_titles[idx].join(','));
	}

package com.qnkj.clouds.modules.VmTwoLayerNetworks.configs;
...
public class ConfigDataConfig extends BaseDataConfig {
	public ConfigDataConfig() {
		/*菜单配置*/
		moduleMenu = new ModuleMenu().builtin(true).program("kubestack").parent("NetworkResources").label("二层网络资源").icon("iconfont icon-kucun").url("/VmTwoLayerNetworks/listview").order(3);

		/*模块配置*/
		tabs = new Tab().modulename("VmTwoLayerNetworks").modulelabel("二层网络资源").tabname("vm_twolayernetworks").iscreate(true).isdelete(true).isenabledisable(true).customview(false).ishidden(true);
		...
		/*模块API接口配置信息*/
		/*api.modulename("VmTwoLayerNetworks");*/

...
public class ConfigDataConfig extends BaseDataConfig {
	public ConfigDataConfig() {
		/*菜单配置*/
		moduleMenu = new ModuleMenu().builtin(true).program("kubestack").parent("NetworkServices").label("负载均衡").icon("iconfont icon-kucun").url("/VmLoadBalances/listview").order(6);

		/*模块配置*/
		tabs = new Tab().modulename("VmLoadBalances").modulelabel("负载均衡").tabname("vm_loadbalances").iscreate(true).isdelete(true).isenabledisable(true).customview(false).ishidden(true);

package com.qnkj.clouds.modules.VmSecurityGroups.configs;
...
public class ConfigDataConfig extends BaseDataConfig {
	public ConfigDataConfig() {
		/*菜单配置*/
		moduleMenu = new ModuleMenu().builtin(true).program("kubestack").parent("NetworkServices").label("安全").icon("iconfont icon-kucun").url("/VmSecurityGroups/listview").order(1);

		/*模块配置*/
		tabs = new Tab().modulename("VmSecurityGroups").modulelabel("安全").tabname("vm_securitygroups").iscreate(true).isdelete(true).isenabledisable(true).customview(false).ishidden(true);
		...
		/*模块API接口配置信息*/
		/*api.modulename("VmSecurityGroups");*/

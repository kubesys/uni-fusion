...
layui.define(['laypage', 'form'], function (exports) {
    ...
    IconHhys.prototype.render = function (options) {
        var opts = options,
            ...
            // 每个图标格子的宽度：'43px'或'20%'
            cellWidth = opts.cellWidth == null ? '36px':opts.cellWidth,

            prefixExp = opts.prefixExp == null ? /fa-var-(.*):/ig : opts.prefixExp,

            prefix = opts.prefix == null ? 'fa-' : opts.prefix,

            fontclass = opts.fontclass == null ? '' : opts.fontclass,
        ...
        var a = {
            init: function () {
                if (type.indexOf("fontClass") > -1){
                    if(url !== null && url !== ""){
                        ...
                        let lcdpUrl  = '/font/iconfont.css';
                        let lcdpData = common.getLcdpData(lcdpUrl);
                        data.push.apply(data,lcdpData);
                        data.push.apply(data,common.getfont[type]())
                    }else {
                        let lcdpUrl  = '/font/iconfont.css';
                        data = common.getLcdpData(lcdpUrl);
                        data.push.apply(data,common.getfont[type]())
                    }
                }else{
                    data = common.getData(url);
            ...
            createSelect: function () {
                ...
                if (limit > 36 ) {
                    ...
                    if(cellWidth.indexOf("%") <= 0){
                        if(cellWidth.toLowerCase().indexOf("px") > 0){
                            cw = parseInt(cellWidth.toLowerCase().replace("px",""),10)
                        }
                    }
                    if(cw > 0) {
                        bodywidth = 'style="width:'+(cw*20 + 2)+'px;"';
                    }
                }
                if (type.indexOf("fontClass") > -1){
                    ...
                    // }
                    if(ORIGINAL_ELEM_VALUE !== '') {
                        if (ORIGINAL_ELEM_VALUE.startsWith("layui")) {
                            oriIcon = '<i class="layui-icon ' + ORIGINAL_ELEM_VALUE + '"></i>';
                        } else if (ORIGINAL_ELEM_VALUE.startsWith("iconfont")) {
                            oriIcon = '<i class="' + ORIGINAL_ELEM_VALUE + '"></i>';
                        } else {
                            oriIcon = '<i class="lcdp-icon ' + ORIGINAL_ELEM_VALUE + '"></i>';
                        }
                    }
                    var selectHtml = '<div class="layui-iconpicker layui-unselect layui-form-select" id="'+ ICON_BODY +'">' +
                        ...
                        '</div>'+
                        '</div>' +
                        '<div class="layui-anim layui-anim-upbit" '+bodywidth+'>' +
                        '</div>';
            ...
            createBody: function () {
                ...
                var bodyHtml = '<div class="layui-iconpicker-body" id="'+ PICKER_BODY +'">' +
                    searchHtml +
                    '<div class="layui-iconpicker-container"><div class="'+ LIST_BOX +'"></div></div> '+
                    '</div>';
            ...
            createList: function (text) {
                ...
                var _limit = limit, // 每页显示数量
                    _pages = l % _limit === 0 ? l / _limit : parseInt(l / _limit,10) + 1, // 总计多少页
                    _id = PAGE_ID;
                ...
                for (var i = 0; i < l; i++) {
                    ...
                    if(type.indexOf("fontClass") >-1){
                        if(obj.startsWith(prefix)){
                            icon += '<i class="'+fontclass+' ' + obj + '" style="font-size: 20px;"></i>';
                        } else if(obj.startsWith('lcdp-icon')){
                            icon += '<i class="' + obj + '" style="font-size: 20px;"></i>';
                        } else {
                            icon += '<i class="layui-icon ' + obj + '" style="font-size: 20px;"></i>';
                        }
                    }else {
                        icon += '<i class="fa ' + obj + '" style="font-size: 20px;"></i>';
                    }
                    icon += '</div>';
                ...
                // 查询出图标后再分页
                l = icons.length;
                _pages = l % _limit === 0 ? l / _limit : parseInt(l / _limit,10) + 1;
                ...
                if (page){
                    ...
                    pageHtml = '<div class="layui-iconpicker-page" id="'+ PAGE_ID +'">' +
                        ...
                        '<i class="layui-icon" id="'+ PAGE_ID +'-clear" clear><a>清除</a></i>  ' +
                        '<i class="layui-icon" id="'+ PAGE_ID +'-prev" data-index="0" prev><a>&#xe603;上一页</a></i>  ' +
                        '<i class="layui-icon" id="'+ PAGE_ID +'-next" data-index="2" next><a>下一页&#xe602;</a></i> ' +
                        '</div>' +
                        '</div>';
                }

                $('#' + ICON_BODY).find('.layui-anim').find('.' + LIST_BOX).html('').append(listHtml);
                $('#' + ICON_BODY).find('.layui-anim').find('.layui-iconpicker-body').append(pageHtml);
                return a;
            },
            // 阻止Layui的一些默认事件
            preventEvent: function() {
                ...
                if(parentElem && parentElem !== "") {
                    a.event('click', parentElem, function (e) {
                        var $icon = $('#' + ICON_BODY);
                        if ($icon.hasClass(selected)) {
                            $icon.removeClass(selected).addClass(unselect);
                        }
            ...
            page: function () {
                ...
                a.event('click', icon, function (e) {
                    var elemtar = e.currentTarget,
                        total = parseInt($('#' +PAGE_ID + '-pages').html()),
                        isPrev = $(elemtar).attr('prev') !== undefined,
                        isClear = $(elemtar).attr('clear') !== undefined,
                        // 按钮上标的页码
                        index = parseInt($(elemtar).attr('data-index')),
                        $cur = $('#' +PAGE_ID + '-current'),
                        // 点击时正在显示的页码
                        current = parseInt($cur.html());
                    if(isClear){
                        $('#' + TITLE_ID).find('.layui-iconpicker-item .layui-iconpicker-icon i').html('').attr('class', "");
        ...
        var common = {
            ...
            getData: function (url) {
                ...
                $.ajax({
                    ...
                    success: function (ret) {
                        var result;
                        while ((result = prefixExp.exec(ret)) != null) {
                            iconlist.push("iconfont " + prefix + result[1]);
                        }
                    },
                    error: function (xhr, textstatus, thrown) {
                        //layui.webframe.alert.msg('图标接口有误!',{iconCu:2})
                    }
                });
                return iconlist;
            },
             ...
             * 获取数据
             */
            getLcdpData: function (url) {
                var iconlist = [];
                var lcdpPrefixExp = /lcdp-icon-(.*):/ig;
                var lcdpPrefix = 'lcdp-icon-';
                $.ajax({
                    url: url,
                    type: 'get',
                    contentType: "application/x-www-form-urlencoded; charset=UTF-8",
                    ...
                    success: function (ret) {
                        var result;
                        while ((result = lcdpPrefixExp.exec(ret)) != null) {
                            iconlist.push("lcdp-icon " + lcdpPrefix + result[1]);
                        }
                    },
                    error: function (xhr, textstatus, thrown) {
                        //layui.webframe.alert.msg('图标接口有误!',{iconCu:2})

...
layui.define('form', function(exports){
  ...
  ,ICON_ADD = 'layui-icon-addition', ICON_SUB = 'layui-icon-subtraction', ELEM_ENTRY = 'layui-tree-entry',
      ELEM_MAIN = 'layui-tree-main', ELEM_TEXT = 'layui-tree-txt', ELEM_PACK = 'layui-tree-pack', ELEM_SPREAD = 'layui-tree-spread'
  ...
  Class.prototype.config = {
    ...
    ,textCheckbox : false //文本点击同时点击复选框，同时只参生oncheck事件
    ,checkboxSubmit: false //复选框是否能提交数据
  ...
  Class.prototype.render = function(){
    ...
    that.checkids = [];
    var cstyle = "<style>.layui-tree .layui-form-radio {padding-right:0;margin:0;}</style>";
    var temp = $('<div class="'+ELEM_VIEW+ (options.showCheckbox ? " layui-form" : "") + (options.showLine ? " layui-tree-line" : "") +'" lay-filter="LAY-tree-'+ that.index +'"></div>');
    that.tree(temp);
    ...
    that.elem.find('.'+ELEM_SET).each(function(){
      var othis = $(this);
      //最外层
      if(!othis.parent('.'+ELEM_PACK)[0]){
        othis.addClass('layui-tree-setHide');
      };

      //没有下一个节点 上一层父级有延伸线
      if(!othis.next()[0] && othis.parents('.'+ELEM_PACK).eq(1).hasClass(ELEM_EXTEND)){
        othis.addClass(ELEM_LINE_SHORT);
      };
      
      //没有下一个节点 外层最后一个
      if(!othis.next()[0] && !othis.parents('.'+ELEM_SET).eq(0).next()[0]){
        othis.addClass(ELEM_LINE_SHORT);
      };
    });
  ...
  Class.prototype.tree = function(elem, children){
    ...
    layui.each(data, function(index, item){
      ...
      ,packDiv = $('<div class="'+ELEM_PACK+'" '+ (item.spread ? 'style="display: block;"' : '') +'></div>')
      ,entryDiv = $(['<div data-id="'+ (item.id || that.dataindex) +'" class="'+ELEM_SET+ (item.spread ? " "+ELEM_SPREAD : "") + (item.checked ? " layui-tree-checkedFirst" : "") +'">'
        ,'<div class="'+ELEM_ENTRY+'">'
          ,'<div class="'+ELEM_MAIN+'">'
            //箭头
            ,function(){
              if(options.showLine){
                if(hasChild){
                  return '<span class="'+ICON_CLICK+' layui-tree-icon"><i class="layui-icon '+ (item.spread ? ICON_SUB : ICON_ADD) +'"></i></span>';
                }else{
                  return '<span class="'+ICON_CLICK+'"><i class="layui-icon layui-icon-file"></i></span>';
                }
              }else{
                return '<span class="'+ICON_CLICK+'"><i class="layui-tree-iconArrow '+ (hasChild ? "": HIDE) +'"></i></span>';
              }
            }()
            
            //复选框
            ,function(){
              return options.showCheckbox ? item.multichoice ? '<input type="checkbox"'+(options.checkboxSubmit ? ' name="'+ (item.field || ('layuiTreeCheck_'+ that.dataindex)) +'"':'')+' data-id="'+ (item.id || that.dataindex) +'" same="layuiTreeCheck" lay-skin="primary" '+ (item.disabled ? "disabled" : "") + (item.value ? ' value="'+item.value+'"' : '') + '>' :
                  '<input type="radio"'+(options.checkboxSubmit ? ' name="'+ (item.field || ('layuiTreeCheck_'+ that.dataindex)) +'"':'')+' data-id="'+ (item.id || that.dataindex) +'" same="layuiTreeCheck" lay-skin="primary" '+ (item.disabled ? "disabled" : "") + (item.value ? ' value="'+item.value+'"' : '') + '>' : '';
            }()
      ...
      if(entryDiv.prev('.'+ELEM_SET)[0]){
        entryDiv.prev().children('.'+ELEM_PACK).addClass(ELEM_SHOW);
      }
      
      //若无子节点，则父节点加延伸线
      if(!hasChild){
        entryDiv.parent('.'+ELEM_PACK).addClass(ELEM_EXTEND);
      }
  ...
  Class.prototype.spread = function(elem, item){
    ...
    elemText.on('click', function(){
      ...
      if(!options.textCheckbox || !options.showCheckbox) {
        //点击产生的回调
        ...
        if(othis.prevAll('input[same="layuiTreeCheck"]')){
          var elemCheckbox = othis.prevAll('input[same="layuiTreeCheck"]');
          if(item.multichoice){
            elemCheckbox.prop('checked',!elemCheckbox.prop('checked'));
            that.setCheckbox(elem, item, elemCheckbox);
          }else{
            that.setRadiobox(elem, item, elemCheckbox);
          }

          var checked = elemCheckbox.prop('checked');
          if(options.checkValueUnique && item.value) {
            that.uniqueSelect();
          }
          //复选框点击产生的回调
  ...
  Class.prototype.setCheckbox = function(elem, item, elemCheckbox){
    ...
    if (typeof item.children === 'object' || elem.find('.' + ELEM_PACK)[0]) {
      elem.find('.' + ELEM_PACK).find('input[same="layuiTreeCheck"]').each(function () {
        if (this.disabled) return; //不可点击则跳过
        this.checked = checked;
      });
    }
    //同步父节点选中状态
    var setParentsChecked = function(thisNodeElem,item){
      //若无父节点，则终止递归
      ...
      //向父节点递归
      setParentsChecked(parentNodeElem,item);
    };

    setParentsChecked(elem,item);
    that.renderForm('checkbox');
    that.renderForm('radio');
  }

  Class.prototype.setRadiobox = function(elem, item, elemCheckbox){
    if(elemCheckbox.prop('disabled')) return;
    var that = this;
    elemCheckbox.prop('checked', !elemCheckbox.data('checked'));
    elemCheckbox.data('checked', !elemCheckbox.data('checked'));
    var checked = elemCheckbox.prop('checked');
    var checkeds = [elemCheckbox[0]];
    var checkChildren = function (elem) {
      var ischeck = false;
      elem.find('.' + ELEM_PACK).find('input[same="layuiTreeCheck"]').each(function (index,obj) {
        if (obj.disabled) return; //不可点击则跳过
        if(checked && !ischeck){
          $(obj).prop('checked', checked);
          $(obj).data('checked', checked);
          ischeck = checked
          checkeds.push(obj);
        }else {
        ...
        checkChildren(elem.find('.'+ELEM_PACK));
      });
    }
    if (typeof item.children === 'object' || elem.find('.' + ELEM_PACK)[0]) {
      checkChildren(elem);
    ...
    var setParentsChecked = function(thisNodeElem,item){
      ...
      if (checked) {
        parentCheckbox.prop('checked', checked);
        parentCheckbox.data('checked', checked);
        checkeds.push(parentCheckbox[0]);
        parentNodeElem.find("."+ELEM_SET).find('input[same="layuiTreeCheck"]').each(function (index,obj){
          if(checkeds.indexOf(obj) === -1){
            $(obj).prop('checked', false);
            $(obj).data('checked', false);
          }
        });
        ...
        //如果兄弟子孙节点全部未选中，则父节点也应为非选中状态
        state || parentCheckbox.prop('checked', false);
        state || parentCheckbox.data('checked', false);
      }
      //向父节点递归
      setParentsChecked(parentNodeElem,item);
    };
  ...
  Class.prototype.uniqueSelect = function () {
    var that = this, checkeds = [];
    that.elem.find('.layui-form-checked,.layui-form-radioed').each(function(){
      checkeds.push($(this).prev()[0]);
    });

    that.elem.find('input[same="layuiTreeCheck"]').each(function (){
      $(this).prop('disabled', false);
      $(this).nextAll('span').removeClass(DISABLED);
    });

    layui.each(checkeds,function (index,obj){
      that.elem.find('input[same="layuiTreeCheck"]').each(function (){
        if(obj !== this && obj.value === this.value) {
          $(this).prop('disabled', $(obj).prop("checked"));
          if($(obj).prop("checked")) {
            $(this).nextAll('span').addClass(DISABLED);
          }else{
            $(this).nextAll('span').removeClass(DISABLED);
          }
        }
    ...
    entry.children('.layui-tree-btnGroup').on('click', '.layui-icon', function(e){
      ...
      if(type === 'add'){
        ...
        if(!packCont[0]){
          ...
          //节点添加子节点容器
          elem.append('<div class="'+ELEM_PACK+'"></div>');
        };
  ...
  Class.prototype.getChecked = function(){
    ...
    //遍历节点找到选中索引
    that.elem.find('.layui-form-checked,.layui-form-radioed').each(function(){
      checkId.push($($(this).prev()[0]).data('id').toString());
    });

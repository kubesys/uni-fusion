package com.qnkj.clouds.modules.VmRoutingResources.configs;
...
public class ConfigDataConfig extends BaseDataConfig {
	public ConfigDataConfig() {
		/*菜单配置*/
		moduleMenu = new ModuleMenu().builtin(true).program("kubestack").parent("NetworkResources").label("路由资源").icon("iconfont icon-kucun").url("/VmRoutingResources/listview").order(5);

		/*模块配置*/
		tabs = new Tab().modulename("VmRoutingResources").modulelabel("路由资源").tabname("vm_routingresources").iscreate(true).isdelete(true).isenabledisable(true).customview(false).ishidden(true);
		...
		/*模块API接口配置信息*/
		/*api.modulename("VmRoutingResources");*/

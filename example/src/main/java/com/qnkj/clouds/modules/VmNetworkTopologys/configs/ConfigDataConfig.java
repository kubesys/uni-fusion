package com.qnkj.clouds.modules.VmNetworkTopologys.configs;
...
public class ConfigDataConfig extends BaseDataConfig {
	public ConfigDataConfig() {
		/*菜单配置*/
		moduleMenu = new ModuleMenu().builtin(true).program("kubestack").parent("NetworkResources").label("网络拓扑").icon("iconfont icon-kucun").url("/VmNetworkTopologys/listview").order(1);

		/*模块配置*/
		tabs = new Tab().modulename("VmNetworkTopologys").modulelabel("网络拓扑").tabname("vm_networktopologys").iscreate(true).isdelete(true).isenabledisable(true).customview(false).ishidden(true);
		...
		/*模块API接口配置信息*/
		/*api.modulename("VmNetworkTopologys");*/

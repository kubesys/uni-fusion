package com.qnkj.clouds.modules.VmZones.configs;
...
public class ConfigDataConfig extends BaseDataConfig {
	public ConfigDataConfig() {
		/*菜单配置*/
		moduleMenu = new ModuleMenu().builtin(true).program("kubestack").parent("Hardwares").label("区域").icon("iconfont icon-kucun").url("/VmZones/listview").order(1);

		/*模块配置*/
		tabs = new Tab().modulename("VmZones").modulelabel("区域").tabname("vm_zones").iscreate(true).isdelete(true).isenabledisable(true).customview(false);
		...
		layouts = Arrays.asList(
			...
			new Layout().block(1).columns(Collections.singletonList("12")).fields(Collections.singletonList("yaml")),
		...
		fields = Arrays.asList(
			...
			new TabField().fieldname("yaml").fieldlabel("连接配置").uitype(33).issort(true),
			new TabField().fieldname("state").fieldlabel("连接状态").uitype(19).picklist("vmzonestate").issort(true),
		...
		customViews = Collections.singletonList(
			new CustomView().viewname("默认").columnlist(Arrays.asList("name","describe","state","status","published"))
		);
		...
		/*弹出对话框配置*/
		popupDialog.search(Collections.singletonList("name")).columns(Arrays.asList("name","describe","state","status","published"));
